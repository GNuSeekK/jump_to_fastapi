...
# Depends는 매개변수로 전달 받은 함수를 실행시킨 결과 리턴, 자동으로 get_db로 생성된 객체가 주입되고 contextmanager가 적용된다. 따라서 @contextlib.contextmanager를 제거해야함
...
def question_vote(_question_vote: question_schema.QuestionDelete,
                  db: Session = Depends(get_db),
                  current_user: User = Depends(get_current_user)):
